...
struct CIRSample {
	...
	*/
	...
	@param imag The imaginary part of the sample
	*/
	CIRSample(int16_t real, int16_t imag) : real(real), imag(imag) {}
	int16_t real;
	int16_t imag;
};
...
public:
	...
	*/
	...
	/*
	Overrides the TX_POWER register (0x1E) directly, after `commitConfiguration()`
	has written tune()'s channel/PRF default - otherwise the override would be
	clobbered by the default the next time configuration is committed. Only
	meaningful in manual power mode (`useSmartPower(false)`, the default): each
	byte is a coarse (DA, 3dB/step, bits 7-5) + fine (mixer, 0.5dB/step, bits 4-0)
	gain pair per DW1000 User Manual section 7.2.31.3. Pass the same byte
	replicated across all four bytes (e.g. 0x6B6B6B6B) to scale TX power
	uniformly, matching how the manual-mode reference table in the datasheet
	(and this library's own tune()) sets all four sub-fields identically.
	*/
	static void setTXPower(int32_t txPower);
