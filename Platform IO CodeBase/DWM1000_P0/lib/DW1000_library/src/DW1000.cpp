...
// SPI settings
//
// _fastSPI is used once the DW1000's PLL clock is confirmed running
// (enableClock(PLL_CLOCK) / AUTO_CLOCK); _slowSPI (2 MHz, below) is only
// used briefly right after reset, before that clock is verified stable.
// 20 MHz is the DW1000's documented absolute maximum SPI clock in normal
// operation (User Manual) - the ESP8266 branch already runs at that limit.
// The ESP32 branch instead uses 16 MHz, 4 MHz below spec, for reasons not
// documented in this library; ESP32's SPI peripheral itself is capable of
// far more than 20 MHz, so it is not a host-side limitation.
//
// To compare 16 MHz vs 20 MHz on the ESP32 (accumulator reads are the
// biggest beneficiary - readCIR() pulls up to 4064 bytes per frame, so a
// faster SPI clock shortens the window the receiver is forced off for),
// swap which line is commented below and reflash BOTH boards - this is a
// shared library file, so it affects tag and anchor together:
#ifdef ESP8266
...
#else
	// const SPISettings DW1000Class::_fastSPI = SPISettings(20000000L, MSBFIRST, SPI_MODE0); // DW1000's spec max - no margin, watch for SPI errors if wiring is long/noisy
	const SPISettings DW1000Class::_fastSPI = SPISettings(16000000L, MSBFIRST, SPI_MODE0);    // current default, 4 MHz of margin below spec
#endif
...
void DW1000Class::setTXPower(int32_t txPower) {
	byte txpower[LEN_TX_POWER];
	writeValueToBytes(txpower, txPower, LEN_TX_POWER);
	writeBytes(TX_POWER, NO_SUB, txpower, LEN_TX_POWER);
}
